    ...
    <template v-if="this.authdetail.name !== undefined">
      ...
      <div v-if="getStartingUrl === 'localhost'">
        ...
        <template v-if="sett.devshow">
          ...
          <br />
          isEnteredselCrlm: {{ isEnteredselCrlm }} <br />dayChainJSON:
...
export default {
  ...
  methods: {
    ...
    async sendUserAgent() {
      const crArr = {
        type: "userAgent",
        name: "createMiscTest" + this.authdetail.username,
    ...
    async updateClrm(id, fname, fval) {
      ...
      try {
        ...
        this.writeFail("updateClrm", this.authdetail.username, JSON.stringify(retval));
      } catch (err) {
    ...
    async writeFail(dest, arr, ret) {
      ...
      try {
        const crArr = {
          ...
          name: this.authdetail.username,
          detail:
            "appFail:" + dest + ", date:" + this.$dayjs().format("YYYY-MM-DD HH:mm.X") + "," + dtl,
        };
        await DataStore.save(new Misc(crArr));
      } catch (err) {
    ...
    async salvageFail() {
      ...
      const crArr = {
        type: "appFailSalvage",
        name: this.authdetail.username,
        detail: this.$dayjs().format("YYYY-MM-DD HH:mm.X") + ", " + arr,
      };
      try {
        await DataStore.save(new Misc({ crArr }));
    ...
    async reloadApp(str) {
      try {
        const crArr = {
          type: "reloadApp",
          name: this.authdetail.username,
          detail: "Staff" + this.$dayjs().format("YYYY-MM-DD HH:mm.X") + ", " + str,
        };
        await DataStore.save(new Misc(crArr));
      } catch (err) {
        this.writeFail(
    ...
    clrmShowCol() {
      ...
      for (var v = 0; v <= Number(finalWeek.lessonnum); v++) {
        switch (this.cRoom.showAttenHist) {
          case 0:
            arr.push(v == Number(this.selCrlm.lssnthisweek) ? true : false);
            break;
          case 1:
            arr.push(v <= Number(this.selCrlm.lssnthisweek) ? true : false);
            break;
