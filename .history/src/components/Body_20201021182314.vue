...
export default {
  ...
  methods: {
    ...
    async APIgetClrmsinstByday(dow) {
      ...
      const gqlEmitDel = gql.data.instByday.items.filter(
        n => n._deleted !== true
      );
      console.warn("gqlDel" + gqlEmitDel.length);
      const gqlEmitDelR = gql.data.instByday.items.filter(
        n => n._deleted === true
      );
      console.warn("gqlDel" + gqlEmitDelR.length);
      console.warn(gqlEmitDelR);
      // 既に保持していた場合除去
      ...
      this.writeNoteLS(
        "APIgetClrmsinstByday: " + gql.data.instByday.items.length
      );
    },
    ...
    async discrepancyDetectAndFix(selClrm, desc) {
      ...
      if (objLS !== null) {
          ...
          if (atLS == null) {
            ...
            if (atCL == null && atDS !== null) {
              ...
              resultAddStr +=
                num +
                " " +
                this.classmembers[num].studentcode +
                " " +
                attnDest +
                " DS -> class\n";
            }
        ...
        this.writeNoteLS(
          "discrepancyDetectAndFix " + selClrm.id + " " + desc + " " + sumr
        );
      } else {
        this.writeNoteLS(
