  ...
  methods: {
    ...
    async idbSetup() {
      ...
      this.yourClasses.forEach(m => {
        ...
        clnum[m.id] = this.getPrevJSON[m.dayofweek].lessonnum;
      });
      ...
      for await (const x of dat) {
        ...
        if (compx !== undefined) {
          let str = "";
          for (let a in this.manage.vforAttn) {
            str += a.at + ",";
            if (a.md > clnum[x.classcode]) {
              break;
            }
          }
          // for(let i=1;i<=clnum[x.classcode]; i++){
          //   str+=x.

          // }
          console.warn(cday[x.classcode], str, compx.index, compx.studentname);
          //  getPrevJSON.Tue.lessonnum }
        } else {
