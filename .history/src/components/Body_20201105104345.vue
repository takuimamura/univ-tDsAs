          ...
          <div class="columns">
            ...
            <div class="column is-four-fifths">
              ...
              <template v-if="sett.devshow">
                ...
                <br />
                getLatestJSON2: {{ getLatestJSON2.Mon.lessonnum }} - {{ getLatestJSON2.Tue.lessonnum }} - {{ getLatestJSON2.Wed.lessonnum }} - {{ getLatestJSON2.Thu.lessonnum }} -{{ getLatestJSON2.Fri.lessonnum }}
                <br />dayChainJSON:
  ...
  computed: {
    ...
    getLatestJSON2: function() {
      const cMon = this.dataset.Cldrs.filter(
        x =>
          x.dayofweek === "Mon" &&
          this.$dayjs(x.date).format("YYYY/MM/DD") <= this.getThisFriday
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cTue = this.dataset.Cldrs.filter(
        x =>
          x.dayofweek === "Tue" &&
