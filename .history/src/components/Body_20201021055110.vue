...
export default {
  ...
  methods: {
    ...
    selectClassroom(arr) {
      ...
      this.discrepancyDetectAndFix(this.selClrm.id);
    },
    scrollTop: function() {
      window.scrollTo({
        top: 0,
    ...
    async loadclassRealtimeBackupAll() {
    ...
    async loadclassRealtimeBackup(classcode) {
      for (var key in localStorage) {
        if (key.match(/classBackupRealtime"+classcode+"/)) {
          const ls = localStorage.getItem(key);
          const lsObj = JSON.parse(ls.substr(ls.indexOf("\n", 0) + 1));
          return lsObj;
          // console.warn(lsObj.length);
