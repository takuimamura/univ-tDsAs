...
export default {
  ...
  methods: {
    ...
    async fetchMiscs() {
      this.ds.clrms = await DataStore.query(Misc, Predicates.ALL);
      this.dataset.Miscs = [];
      this.dataset.Miscs = [...this.ds.clrms];
      // this.dataset.Clrms.splice();
      // this.dataset.Clrms.push(...this.ds.clrms);
    },
    async fetchClrmsChk() {
    ...
    async DEVcreateMisc() {
      const cr = {
        ...
        detail: JSON.stringify({
          oldest: this.$dayjs().format("H:mm:ss"),
          newest: this.$dayjs().format("H:mm:ss")
        })
      };
      try {
    ...
    async DEVcreateMisc2() {
      this.ds.clrms = await DataStore.query(Misc, c =>
        c.id("eq", "f18cf447-1784-4453-b050-63fc9581c291")
      );
    },
