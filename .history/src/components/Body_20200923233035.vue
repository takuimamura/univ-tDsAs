...
export default {
  ...
  methods: {
    ...
    async devClassSummary(classcode) {
      ...
      // .map((m) => {
      //   return m._lastChangedAt;
      // });
      const mx = ret.reduce((a, b) => (a._lastChangedAt > b._lastChangedAt ? a : b))._lastChangedAt;
      const mn = ret.reduce((a, b) => (a._lastChangedAt < b._lastChangedAt ? a : b))._lastChangedAt;
    ...
    goEvalUpTarget(val) {
      //スターの確定
      this.updateClrm(this.cRoom.evalUpTargetRow.id, this.cRoom.evalUpTargetCol, val);
    },
    // 評価に満たない（出席
