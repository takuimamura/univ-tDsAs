  ...
  <div id="app">
    ...
    <template v-if="this.authdetail.name !== undefined">
      ...
      <div v-if="getStartingUrl === 'localhost'">
        ...
        <ul>
          <li
            v-for="ins in instructor.yourattendances"
            :key="ins.id"
          >{{ ins.date }}- {{ ins.uid }}- {{ ins.clockin }}- {{ ins.clockout }}- {{ ins.detail }}</li>
        </ul>--c2--
        <br />
        <ul>
                          ...
                          <template v-if="!isClrmAppSyncUploading">
                            ...
                            >
                              ...
                              <template v-if="ClrmAppSyncState">
                                ...
                                />
                                <span
                                  class="is-text-2 has-text-weight-bold is-syncdone"
                                >Sync Success</span>
                              </template>
                              <template v-else>
                                <b-icon
  ...
  methods: {
    ...
    async createInstAPI(crArr, msgg, typ, siz) {
      ...
      try {
        ...
        this.$buefy.toast.open({
          message:
            "<span style='font-size:60px'>Connection failed. please try again</span>",
          type: "is-danger",
          size: "is-large",
          duration: 5000
        });
