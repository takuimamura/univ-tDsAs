              ...
              >
                <template slot-scope="props">
                  <b-table-column
                    ...
                    :class="[getIfAttnThisWeekNotNull(selCrlm.dayofweek,props.row[selCrlm.attnthisweek],props.row._lastChangedAt) ? 'is-synced-bg' : '',
                                        ]"
                    width="10"
                    sticky
                  >
                    ...
                    <span
                      v-show="getIfAttnThisWeekNotNull(selCrlm.dayofweek,props.row[selCrlm.attnthisweek],props.row._lastChangedAt)"
                    >
                      <b-icon pack="fas" icon="star" size="is-small" />
                    </span>
                  </b-table-column>
...
const syncedsum = ret.reduce((accumulator, current) => {
  return accumulator +  (this.getIfAttnThisWeekNotNull(dow,current[getThisWeekAttnJSON[dow]],current._lastChangedAt)  === true ? 1 : 0);
}, initialValue)

const attmsum = ret.reduce((accumulator, current) => {
  return accumulator + (current[getThisWeekAttnJSON[dow]] !== null ? 1: 0)
}, initialValue)

      const chk = ret.forEach(c) => {}
      // tgt.attndone =
      // tgt.syncdone =
    },
    ...
    getIfAttnThisWeekNotNull(dow, attn, lastChan) {
      console.log(dow, attn, lastChan);
      //当日更新なら時刻、違えば日付
      // getThisWeekDateJSON[dow]
      // return false;
      return this.$dayjs(lastChan).format("H:mm") == "Invalid Date"
        ? false
        : this.$dayjs(this.getThisWeekDateJSON[dow]).format("MMDD") <=
            this.$dayjs(lastChan).format("MMDD") && attn !== null;
