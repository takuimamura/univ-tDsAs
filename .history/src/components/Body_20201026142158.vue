  ...
  methods: {
    ...
    async updateClrmAttnHW(row) {
      ...
      if (row[this.selClrm.attnthisweek] !== null && row[this.selClrm.attnthisweek] !== "") {
        console.warn("yes:" + row.studentname + " " + this.selClrm.attnthisweek " " + row[this.selClrm.attnthisweek]);
        const clrmItem = await DataStore.query(Clrm, row.id);
        ...
        console.warn("no:" + row.studentname + " " + this.selClrm.attnthisweek+ " " + row[this.selClrm.attnthisweek]);
      }
    },
    //// check クラス全員チェック
