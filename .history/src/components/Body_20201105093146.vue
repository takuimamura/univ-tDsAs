...
export default {
  ...
  computed: {
    ...
    getLatestJSON2: function() {
      const cMon = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Mon" && x.weeknum <= this.getTodayJSON.weeknum
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cTue = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Tue" && x.weeknum <= this.getTodayJSON.weeknum
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cWed = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Wed" && x.weeknum <= this.getTodayJSON.weeknum
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cThu = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Thu" && x.weeknum <= this.getTodayJSON.weeknum
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cFri = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Fri" && x.weeknum <= this.getTodayJSON.weeknum
      ).reduce((a, b) => (a.date > b.date ? a : b));
      return { Mon: cMon, Tue: cTue, Wed: cWed, Thu: cThu, Fri: cFri };
    },
