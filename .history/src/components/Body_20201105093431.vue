...
export default {
  ...
  computed: {
    ...
    getLatestJSON2: function() {
      const cMon = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Mon" && x.lessonnum <= this.getTodayJSON.lessonnum
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cTue = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Tue" && x.lessonnum <= this.getTodayJSON.lessonnum
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cWed = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Wed" && x.lessonnum <= this.getTodayJSON.lessonnum
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cThu = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Thu" && x.lessonnum <= this.getTodayJSON.lessonnum
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cFri = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Fri" && x.lessonnum <= this.getTodayJSON.lessonnum
      ).reduce((a, b) => (a.date > b.date ? a : b));
      return { Mon: cMon, Tue: cTue, Wed: cWed, Thu: cThu, Fri: cFri };
    },
