        ...
        <div v-if="sett.devscreen">
          ...
          <ul>
            <li v-for="r in TESTarr1" :key="r.s">
              {{ r.classcode }} - {{ r.studentname }} 2 {{ r.attn02 }} 3 {{ r.attn03 }} 4 {{ r.attn04 }} -{{
          ...
          <b-button @click="dosomething()">test()</b-button>
          <b-button
            @click="
  ...
  computed: {
    ...
    TESTarr4() {
      if (this.selClrm != [] && this.dataAPI.Clrms) {
        return this.dataAPI.Clrms[0]
          .filter(x => x.classcode === this.selClrm.id)
          .sort(function(a, b) {
            if (a.sortid < b.sortid) return -1;
            if (a.sortid > b.sortid) return 1;
            return 0;
