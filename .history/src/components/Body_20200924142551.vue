  ...
  methods: {
    ...
    async initallClasses() {
      const allClassesUp = await this.dataset.allClasses.forEach(m => {
        ...
        console.warn(this.queryMiscClassSummary(m.classcode));
        // m.newest = this.queryMiscClassSummary(m.classcode);
      });
      await this.dataset.allClasses = [...allClassesUp];
      await this.dataset.allClasses = [];
      await this.sett.dummy1 = [...allClassesUp];
      console.warn("dummy1");
