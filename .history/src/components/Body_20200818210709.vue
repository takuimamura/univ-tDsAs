                ...
                <tbody class="cPointer">
                  <tr>
                    ...
                    <td
                      ...
                      :class="[ manage.selCrlmDv[m.at] !==  selCrlm[m.at]  ? 'sumNg' : selCrlm[m.at] === selCrlm.students ? 'sumOk' : selCrlm[m.at] > 0 ? 'sumYet' : '' ]"
                    ...
                    <td
                      ...
                      :key="`fourth-${index}`"
                      style="padding:0px 10px 0px 15px;"
                      :class="[ manage.selCrlmDv[n.ev] + manage.selCrlmDv[n.cm] !== selCrlm[n.ev] + selCrlm[n.cm] ? 'sumNg' : selCrlm[n.ev] + selCrlm[n.cm] === selCrlm.students * 2 ? 'sumOk' : selCrlm[n.ev] + selCrlm[n.cm] > 0 ? 'sumYet' : '' ]"
                  ...
                  <tr style="background-color:#eee;">
                    ...
                    <td
                      ...
                      :key="`third-${index}`"
                      style="padding:0px 10px 0px 15px;"
                      :class="[ manage.selCrlmDv[m.at] !==  selCrlm[m.at]  ? 'sumNg' : manage.selCrlmDv[m.at] === manage.selCrlmDv.students ? 'sumOk' : manage.selCrlmDv[m.at] > 0 ? 'sumYet' : '' ]"
                      v-show=" (manage.showNumClassesSum % 2 === 0)"
                    >
                    ...
                    <td
                      ...
                      :key="`fourth-${index}`"
                      style="padding:0px 10px 0px 15px;"
                      :class="[ manage.selCrlmDv[n.ev] + manage.selCrlmDv[n.cm] !== selCrlm[n.ev] + selCrlm[n.cm] ? 'sumNg' : manage.selCrlmDv[n.ev] + manage.selCrlmDv[n.cm] === manage.selCrlmDv.students * 2 ? 'sumOk' : manage.selCrlmDv[n.ev] + manage.selCrlmDv[n.cm] > 0 ? 'sumYet' : '' ]"
                      v-show=" (manage.showNumClassesSum > 0)"
                    >
              ...
              <tbody class="cPointer">
                <tr v-for="yitem in yourClasses" :key="yitem.id">
                  ...
                  <td
                    ...
                    :class="[ yitem[m.at] === yitem.students ? 'sumOk' : yitem[m.at] > 0 ? 'sumYet' : '' ]"
                    v-show="cRoom.showClassesSum && (manage.showNumClassesSum % 2 === 0)"
                    @click="selectClassroom(yitem)"
                  >
                  ...
                  <td
                    ...
                    :key="`fourth-${index}`"
                    style="padding:5px 10px 0px 15px;"
                    :class="[ yitem[n.ev] + yitem[n.cm] === yitem.students * 2 ? 'sumOk' : yitem[n.ev] + yitem[n.cm] > 0 ? 'sumYet' : '' ]"
                    v-show="cRoom.showClassesSum && (manage.showNumClassesSum > 0)"
                    @click="selectClassroom(yitem)"
                  >
                                    ...
                                    <template v-if="k.title !== 'Homework'">
                                      ...
                                      <td>
                                        <star-rating
                                          ...
                                          @current-rating="setEvalUpTarget($event, indiRow, k.evl, cRoom.indiNo)"
                                        ></star-rating>
                                ...
                                <template v-if="k.title !== 'ALC Adademy NEXT'">
                                  ...
                                  <template v-if="k.title !== 'Homework'">
                                    ...
                                    <td class="title is-4">
                                      <span
                                        :class="[
                                        indiRow[k.evl] === 0 ? 'has-pink-strong' : 'has-text-black',
                                      ]"
                                      >{{ indiRow[k.evl] }}</span>
                                    </td>
                                    <td>
                        ...
                        <section class="columns">
                          ...
                          <div class="column">
                            <star-rating
                              ...
                              :star-points="cRoom.customstarShape"
                              @rating-selected="goEvalUpTarget"
                              @current-rating="setEvalUpTarget($event,
                                indiRow,
                          ...
                          <div class="column">
                            ...
                            >
                              <span
                                ...
                                class="has-text-grey-light"
                              >{{indiRow[cRoom.evalCriItems[cRoom.tgtEvalSingle].comm]| description(50)}}</span>
                              ...
                              <div class="tile is-child">
                                <b-field>
                                  <b-radio-button
                                    ...
                                    type="attend-present"
                                    size="is-medium"
                                    @input="updateClrmEdit(props.row.id,
                                                          props.row.index,
                                                          manage.selAttn,
                                                          'here',
                                                          props.row.attn19)"
...
// import { createAttend } from "../graphql/mutations";
// import { listClrms } from "../graphql/queries";
...
export default {
  ...
  methods: {
    ...
    // そもそもsubscribe全くしてない
    subscribe() {
      API.graphql(graphqlOperation(onCreateInst)).subscribe({
        next: eventData => {
          const cInst = eventData.value.data.onCreateInst;
          this.instructor.creInst.push(cInst);
        }
      });
      API.graphql(graphqlOperation(onUpdateInst)).subscribe({
        next: eventData => {
          const uInst = eventData.value.data.onUpdateInst;
          this.instructor.upInst.push(uInst);
        }
      });

      API.graphql(graphqlOperation(onUpdateMisc)).subscribe({
        next: eventData => {
          const uInst = eventData.value.data.onUpdateMisc;
          this.dataset.upMiscs.push(uInst);
        }
      });
    },
    dummytest() {
    ...
    async manageSummary() {
      ...
      // if (this.showManagementView === true) {
      // this.getClrmsDatainstByday(this.dayjsddd); //★test

      // const groupsum = this.dataset.ClrmsInstByday.reduce(function (result, current) {
      //   var element = result.find(function (p) {
      //     return p.classcode === current.classcode
      //   });
      //   if (element) {
      //     element.count ++; // count
    ...
    enterClassroom() {
      ...
      this.classmembers.forEach(function(mm) {
        mm.eval01 =
          mm.eval01 === null || mm.eval01 === undefined
            ? null
            : Number(mm.eval01);
        mm.eval02 =
          mm.eval02 === null || mm.eval02 === undefined
            ? null
            : Number(mm.eval02);
        mm.eval03 =
          mm.eval03 === null || mm.eval03 === undefined
            ? null
            : Number(mm.eval03);
        mm.eval04 =
          mm.eval04 === null || mm.eval04 === undefined
            ? null
            : Number(mm.eval04);
        mm.eval05 =
          mm.eval05 === null || mm.eval05 === undefined
            ? null
            : Number(mm.eval05);
        mm.eval06 =
          mm.eval06 === null || mm.eval06 === undefined
            ? null
            : Number(mm.eval06);
        mm.eval07 =
          mm.eval07 === null || mm.eval07 === undefined
            ? null
            : Number(mm.eval07);
        mm.eval08 =
          mm.eval08 === null || mm.eval08 === undefined
            ? null
            : Number(mm.eval08);
        mm.eval09 =
          mm.eval09 === null || mm.eval09 === undefined
            ? null
            : Number(mm.eval09);
        mm.eval10 =
          mm.eval10 === null || mm.eval10 === undefined
            ? null
            : Number(mm.eval10);
        mm.eval11 =
          mm.eval11 === null || mm.eval11 === undefined
            ? null
            : Number(mm.eval11);
        mm.eval12 =
          mm.eval12 === null || mm.eval12 === undefined
            ? null
            : Number(mm.eval12);
