<template>
  <div id="app">
    <div v-if="getStartingUrl === 'localhost'">
      ...
      ClrmsChk::{{ dataset.ClrmsChk.length }}
      <br />
      <!-- {{indiRow}} -->
      <!--
          ...
          <template v-if="!showManagementView">
            <section class="p40">
              ...
              <b-collapse :open="false" aria-id="contentIdForA11y1">
                <button class="button is-primary" slot="trigger" aria-controls="contentIdForA11y1">
                  Students search
                </button>
        ...
        <b-tab-item label="Schedule" icon="calendar-alt">
          ...
          <section class="columns is-centered" style="font-size: 16px; padding: 20px 10px;">
            ...
            <div class="column f18" style="padding:20px 0px 0px 0px;">
              <b-field>
                <b-switch v-model="cRoom.showClassesSum" @input="chkClassesSum"
                  >Total Counts</b-switch
                >
            ...
            <div class="column f18">
              <b-notification :closable="false">
                <template v-if="cRoom.showClassesSum">
                  ...
                  ></b-loading>
                  <b-field>
                    <b-radio-button :native-value="0" v-model="manage.showNumClassesSum"
                      >Attendance</b-radio-button
                    >
                    <b-radio-button :native-value="1" v-model="manage.showNumClassesSum"
                      >Evaluation</b-radio-button
                    >
                    <b-radio-button :native-value="2" v-model="manage.showNumClassesSum"
                      >Both</b-radio-button
                    >
                  </b-field>
                </template>
                ...
                <b-table-column field="homeworkincomplete" label="Homework Incomplete" width="30">
                  <div class="tile is-ancestor">
                    <!-- <div class="tile is-parent">
                      <div class="tile is-child">
                        <b-field>
                          <b-checkbox-button
                            v-model="props.row.homeworkincomplete"
                            :type="props.row.homeworkincomplete20 == 0 ? '' : 'is-danger'"
                            rounded
                            disabled
                          >
                    ...
                    <div>
                      <div class="tile is-child">
                        <!-- <div class="tile is-child" v-show="att.mode != 3"> -->
                        <b-field>
                  ...
                  <!-- <div class="tile is-ancestor">
                    <div class="tile is-parent">
                      <div class="tile is-child">
                        <b-field
                          :label="att.note[3] + `(Comment required)`"
                          v-show="props.row.col2 == att.note[3]"
                        >
                          <b-input
                            maxlength="200"
                            type="textarea"
                  ...
                  <!-- <div class="py-3 px-3" :class="item.col5">
                        <b-radio
                          :label="props.row.col2"
                          :color="item.col5"
                          :native-value="item.col2"
                          @click="
                              props.row.col5 = props.row.col7;
                              props.row.col8 = props.row.col9;
                            "
                        >{{props.row.col2}}</b-radio>
                      </div>
                      <div class="py-3 px-3" :class="item.col8">
                        <b-radio
                          :label="item.col3"
                          :color="item.col8"
                          :value="item.col3"
                          @click="
                              item.col8 = item.col10;
                              item.col5 = item.col6;
                            "
                        ></b-radio>
                  </div>-->
              ...
              <b-tab-item label="Instructors" :visible="showManagementViewSuper" icon="user-clock">
                <section class="manageview3 p40">
                  ...
                  <section v-if="manage.checkedRowsInst.length > 0">
                    ...
                    >
                      <template slot-scope="props">
                        <b-table-column field="id" label="Name" sticky>{{
                          props.row.id
                        }}</b-table-column>
                        <b-table-column field="date" label="Date" sticky>{{
                          props.row.date
                        }}</b-table-column>
                ...
                <section class="manageview p40">
                  <b-field grouped>
                    <b-field>
                      ...
                      <b-radio-button v-model="manage.dow" native-value="Mon" size="is-medium"
                        >Mon</b-radio-button
                      >
                      <b-radio-button v-model="manage.dow" native-value="Tue" size="is-medium"
                        >Tue</b-radio-button
                      >
                      <b-radio-button v-model="manage.dow" native-value="Wed" size="is-medium"
                        >Wed</b-radio-button
                      >
                      <b-radio-button v-model="manage.dow" native-value="Thu" size="is-medium"
                        >Thu</b-radio-button
                      >
                      <b-radio-button v-model="manage.dow" native-value="Fri" size="is-medium"
                        >Fri</b-radio-button
                      >
                    </b-field>
                  ...
                  <section v-if="manage.checkedRows.length > 0">
                    ...
                    >
                      <template slot-scope="props">
                        <b-table-column field="dayofweek" label="Day" sticky>{{
                          props.row.dayofweek
                        }}</b-table-column>
                        <b-table-column field="classcode" label="Class" sticky>{{
                          props.row.classcode
                        }}</b-table-column>

                        <b-table-column field="studentcode" label="Code" sticky>{{
                          props.row.studentcode
                        }}</b-table-column>
...
export default {
  ...
  methods: {
    ...
    async updateClrm(id, upd) {
      console.warn("up:" + id, "upd:" + upd);
      const clrmItem = await DataStore.query(Clrm, id);
      this.clrmUp = clrmItem.index + " " + clrmItem.classcode + " " + clrmItem.studentcode;
      console.warn(clrmItem);
      await DataStore.save(
        Clrm.copyOf(clrmItem, (updated) => {
          updated.classcode = upd.classcode;
        })
      );
    ...
    writeFail(dest, arr, ret) {
      localStorage["appFail" + dest + this.$dayjs().format("YYYY-MM-DD HH:mm.X")] =
        this.getStartingUrl +
        ", auth:" +
    ...
    async getClrmsDatainstByday(dow) {
      ...
      this.dataset.ClrmsInstByday.push(...ClrmsDatainstBydayData.data.instByday.items);
    },
    ...
    async listClrmsDataIDCheck(idstr) {
      ...
      try {
        ...
        if (this.sumClrmsChk()) {
          ...
          this.manage.chkStatus = "<span style='color:red'>Check not properly finished</span>";
        }
      } catch (err) {
    ...
    async listInstsData() {
      console.warn("xx");
      const InstsData = null;
      //$$$$$         co      const InstsData = await API.graphql(
      ...
      this.instructor.yourattendances = this.instructor.attendances.filter(
        (x) => x.id === this.authdetail.username
      );
    },
      ...
      if (this.att.mode === 3) {
        this.$buefy.dialog.confirm({
          ...
          onConfirm: () => {
            ...
            this.$buefy.toast.open({
              message: "<span style='font-size:40px'>You can edit. " + "</span>",
              type: "is-beige",
              size: "is-large",
            });
          },
    ...
    // getShowStar(p1ptshow, comm) {
    //   // console.log(
    //   //   "getShowStar(p1ptshow, comm):" +
    //   //     comm.substr(0, 6).toLowerCase() +
    //   //     "," +
    //   //     comm
    //   // );
    //   return p1ptshow == false
    //     ? false
    //     : comm === undefined
    //     ? true
    //     : comm === null
    //     ? true
    //     : comm.substr(0, 6).toLowerCase() === "absent"
    //     ? false
    //     : true;
    // },
    checkifAbsent(comm) {
    ...
    convTextIfNeeded(val) {
      switch (val) {
        case this.cRoom.evalCriSubItems.w1[0].text:
          return "Involv. & Improv. 1 (7)";
        case this.cRoom.evalCriSubItems.w1[1].text:
          return "Involv. & Improv. 2 (14)";
        default:
          return val;
      }
    },
    ...
    //       }
    //     },
    //     getConsistencyTypeEval(eval, comm) {

    //     },
    //     checkIfConsistencyError(row) {
    // const arrCon = [
    //         {at: "attn06",ev: "eval06",cm: "ecom06"},
    //         {at: "attn09",ev: "eval07",cm: "ecom07"},
    //         {at: "attn07",ev: "eval08",cm: "ecom08"},
    //         {at: "attn10",ev: "eval09",cm: "ecom09"},
    //         {at: "attn12",ev: "eval10",cm: "ecom10"},
    //         {at: "attn14",ev: "eval11",cm: "ecom11"}
    // ];
    //     },
    selectClassroom(arr) {
      //セット
    ...
    enterClassroomPrevSeeIfAlter() {
      ...
      if (dt !== undefined && this.selCrlm.dayofweek == this.$dayjs(dt.detail).format("ddd")) {
    ...
    enterClassroom() {
      ...
      const val_null = null;
      const attname = this.getTodayJSON.hwic;
      const dchain = this.getDayChainUntilPrevJSON;
      this.classmembers.forEach(function(mm) {
        mm.eval01 = mm.eval01 === null || mm.eval01 === undefined ? null : Number(mm.eval01);
        mm.eval02 = mm.eval02 === null || mm.eval02 === undefined ? null : Number(mm.eval02);
        mm.eval03 = mm.eval03 === null || mm.eval03 === undefined ? null : Number(mm.eval03);
        mm.eval04 = mm.eval04 === null || mm.eval04 === undefined ? null : Number(mm.eval04);
        mm.eval05 = mm.eval05 === null || mm.eval05 === undefined ? null : Number(mm.eval05);
        mm.eval06 = mm.eval06 === null || mm.eval06 === undefined ? null : Number(mm.eval06);
        mm.eval07 = mm.eval07 === null || mm.eval07 === undefined ? null : Number(mm.eval07);
        mm.eval08 = mm.eval08 === null || mm.eval08 === undefined ? null : Number(mm.eval08);
        mm.eval09 = mm.eval09 === null || mm.eval09 === undefined ? null : Number(mm.eval09);
        mm.eval10 = mm.eval10 === null || mm.eval10 === undefined ? null : Number(mm.eval10);
        mm.eval11 = mm.eval11 === null || mm.eval11 === undefined ? null : Number(mm.eval11);
        mm.eval12 = mm.eval12 === null || mm.eval12 === undefined ? null : Number(mm.eval12);
        ...
        // 宿題
        // mm.homeworkincomplete20 =
        // (mm.homeworkincomplete === false || mm.homeworkincomplete === "false"
        //   ? 1
        //   : 0) +
        // (mm.attn20 === false || mm.attn20 === "false" ? 1 : 0) +
        // (mm.homeworkincomplete04 === false ||
        // mm.homeworkincomplete04 === "false"
        //   ? 1
        //   : 0) +
        // (mm.homeworkincomplete05 === false ||
        // mm.homeworkincomplete05 === "false"
        //   ? 1
        //   : 0);

        mm.homeworkincomplete20 = dchain.reduce((sum, c) => {
          // mm.hwicAll = dchain.reduce((sum, c) => {
          return (
            sum +
            (mm[c.attendance] === "not here" || mm[c.hwic] === false || mm[c.hwic] === "false"
              ? 1
              : 0)
          );
        }, 0);
    ...
    getDateMD(num) {
      ...
      return days.includes(this.selCrlm.dayofweek)
        ? this.$dayjs(this.dayChainJSON[this.selCrlm.dayofweek][num].date).format("M/D")
        : "--";
    },
    getDateMDddd(val) {
    ...
    allStudentsFiltered() {
      ...
      if (this.showManagementViewSuper) {
        if (this.manage.dow == "All") {
          ...
          filtered = this.dataset.ClrmsInstByday.filter((x) => x.dayofweek === this.manage.dow);
        }
        ...
        if (this.manage.dow == "All") {
          filtered = this.dataset.ClrmsInstByday.filter((x) => x.id !== "dummy instructor");
        } else {
  ...
  async created() {
    ...
    this.setcurrentAcDate();
    this.setInstMonth();
    // this.getCinfsData(); // 謎の取得できないやつ
    // this.getCldrsData();

    this.manageClrms();

    // this.getInstsData(this.authdetail.username);
    this.listInstsData(); //今のところ全件とる
