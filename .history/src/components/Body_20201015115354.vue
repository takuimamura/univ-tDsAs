...
export default {
  ...
  computed: {
    ...
    getEditableUntilJSON: function() {
      const cond = v => v.date < this.getTodayJSON.date;
      // const uMon = this.dayChainJSON.Mon.reduce(
      //   (prev, current) => cond(current),
      //   0
      // );

      const cMon = this.dayChainJSON.Mon.reduce((prev, current) => {
        if (cond(current)) {
          prev.push(current);
        }
        return prev;
      }, []).reduce((a, b) => (a.date > b.date ? a : b));
      const cTue = this.dayChainJSON.Tue.reduce((prev, current) => {
        if (cond(current)) {
          prev.push(current);
        }
        return prev;
      }, []).reduce((a, b) => (a.date > b.date ? a : b));
      const cWed = this.dayChainJSON.Wed.reduce((prev, current) => {
        if (cond(current)) {
          prev.push(current);
        }
        return prev;
      }, []).reduce((a, b) => (a.date > b.date ? a : b));
      const cThu = this.dayChainJSON.Thu.reduce((prev, current) => {
        if (cond(current)) {
          prev.push(current);
        }
        return prev;
      }, []).reduce((a, b) => (a.date > b.date ? a : b));
      // const cFri = this.dataset.Cldrs.filter(x => {
      //   // x.date < this.getTodayJSON.date && x.dayofweek === "Fri";
      //   // x.date < "2020/11/05" && x.dayofweek === "Fri";
      //   x.dayofweek === "Fri";
      // }); //.reduce((a, b) => (a.date > b.date ? a : b));
      const cFri = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Fri" && x.date < "2020/11/05"
      );
