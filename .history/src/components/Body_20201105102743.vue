...
export default {
  ...
  computed: {
    ...
    getThisFriday() {
      ...
      const yyyymmdd = this.$dayjs(ret).format("YYYY-MM-DD");
      return yyyymmdd.replace("-", "/"); // dayjsの文字列変換が他に思いつかず
    },
    ...
    getLatestJSON2: function() {
      const cMon = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Mon" && x.date <= this.getThisFriday
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cTue = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Tue" && x.date <= this.getThisFriday
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cWed = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Wed" && x.date <= this.getThisFriday
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cThu = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Thu" && x.date <= this.getThisFriday
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cFri = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Fri" && x.date <= this.getThisFriday
      ).reduce((a, b) => (a.date > b.date ? a : b));
      return { Mon: cMon, Tue: cTue, Wed: cWed, Thu: cThu, Fri: cFri };
    },
