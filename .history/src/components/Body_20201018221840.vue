    ...
    async writeNoteLS(str) {
      ...
      try {
        const crArr = {
          type: "writeNote",
          name: this.authdetail.username,
          detail: this.getDateYYYYMMDDhHHMMSS() + " " + str + "\n",
        };
        await DataStore.save(new Misc(crArr));
      } catch (err) {
