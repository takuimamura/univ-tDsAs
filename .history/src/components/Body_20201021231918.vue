...
export default {
  ...
  methods: {
    ...
    async checkAttnHWConsistency(classcode, dow) {
      ...
      let resultStr = "";
      const at = rw[this.getThisWeekAttnJSON[dow]];
      const hw = rw[this.getThisWeekHwicJSON[dow]];
      // let resultAddStr = "";
      // 対象のLessonNo.のみをチェック
      // let chk = false;
      ////////DataStore
      // const classmem = this.dataset.Clrms.filter((x) => x.classcode === classcode);
      const chkHW = function(obj, at, hw) {
        let rslt = false;
        let rsltStr = "";
        for (const rw of obj) {
          resultStr +=
      ...
      const retArr = chkHW(classmem);
      ////////LocalStorage

      ////////ログ

      console.warn(retArr[1]);
      return retArr[0];
    },
    // Force Sync
    // Force Sync
