          ...
          <div class="columns">
            ...
            <div class="column is-four-fifths">
              ...
              <template v-if="sett.devshow">
                ...
                <br />
                getLatestJSON: {{ getLatestJSON }}
                <br />
                getLatestJSON2: {{ getLatestJSON2 }}
                <br />dayChainJSON:
...
export default {
  ...
  computed: {
    ...
    getThisFriday() {
      ...
      const offset = onejan.getDay() + 3; // Friday
    ...
    getLatestJSON2: function() {
      const cMon = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Mon" && this.$dayjs(x.date).format("YYYY/MM/DD"); <= this.getThisFriday
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cTue = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Tue" && this.$dayjs(x.date).format("YYYY/MM/DD"); <= this.getThisFriday
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cWed = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Wed" && this.$dayjs(x.date).format("YYYY/MM/DD"); <= this.getThisFriday
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cThu = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Thu" && this.$dayjs(x.date).format("YYYY/MM/DD"); <= this.getThisFriday
      ).reduce((a, b) => (a.date > b.date ? a : b));
      const cFri = this.dataset.Cldrs.filter(
        x => x.dayofweek === "Fri" && this.$dayjs(x.date).format("YYYY/MM/DD"); <= this.getThisFriday
      ).reduce((a, b) => (a.date > b.date ? a : b));
      return { Mon: cMon, Tue: cTue, Wed: cWed, Thu: cThu, Fri: cFri };
    },
